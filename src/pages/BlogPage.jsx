...
export default function BlogPage() {
  ...
  const { getAllBlogs, getBlogBySlug, currentBlog, isLoading, deleteBlog } = useBlogStore();
  ...
  return currentBlog && !isLoading ? (
    <div className="my-12 w-full text-secondary dark:text-[#c9d1d9]">
      ...
      <div aria-label="blog-body" className="my-12">
        ...
        <img aria-label="blog-thumbani" className="max-h-84 h-auto object-cover mx-auto my-4 rounded-md" src={currentBlog.image} alt={currentBlog.title} />
        ...
        <p className="text-secondary/60  text-sm mt-2">
          {new Date(currentBlog.createdAt).toLocaleDateString()} by <span className="font-semibold">{currentBlog.username}</span> - {currentBlog.readingTime} min read
        </p>
        <p className="text-secondary/80 mt-4">{currentBlog.excerpt}</p>
        <div className="mt-6" data-color-mode={theme}>
          <MDEditor.Markdown className="!bg-transparent" source={currentBlog.content} />
        </div>
      </div>
    </div>
  ) : (
